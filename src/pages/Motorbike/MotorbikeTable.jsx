    ...
    import { useMotorbikeList } from "../../queries/Motorbike.queries";

    import ModelJsonSchema from "./Motorbike.schema.json";
    
    const tableSchema = ModelJsonSchema.actions.find(
      (action) => action.category === "listing"
    );
    
    const MotorbikeTable = ({
      mockData = [],
      onAdd,
      onEdit,
      onView,
      ...
      const { error, data, isLoading } = useMotorbikeList({
